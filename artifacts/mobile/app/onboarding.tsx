...
// blinking eyes that wander, gentle bob. Ported from the
// `World_Is_Changing` reference bundle.
const EARTH_OCEAN = "#7fc6e8";
const EARTH_LAND = "#9ad65c";
const EARTH_OUTLINE = "#1f4e6b";
const EARTH_SHADOW = "#a8d8f0";
  ...
  return (
    <View style={{ width: size, height: size }}>
      <Svg width={size} height={size} viewBox="0 0 320 320">
        ...
        {/* drifting clouds in front of/around the earth */}
        <EarthCloud x={cAx} y={cAy} scale={0.95} />
        <EarthCloud x={cBx} y={cBy} scale={0.85} />
        <EarthCloud x={cCx} y={cCy} scale={0.7} />

        {/* EARTH BODY — gentle bob */}
        <G transform={`translate(0 ${bob.toFixed(2)})`}>
          {/* heavy outer ring */}
          <Circle cx={160} cy={160} r={114} fill={EARTH_OUTLINE} />
          {/* ocean base */}
          <Circle cx={160} cy={160} r={108} fill={EARTH_OCEAN} />

          {/* continents, clipped to ocean ball */}
          ...
          {/* face — eye whites blink, pupils wander */}
          <Ellipse
            cx={144}
            cy={152}
            rx={9}
