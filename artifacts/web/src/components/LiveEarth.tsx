...
function EarthCloud({ x, y, scale = 1 }: { x: number; y: number; scale?: number }) {
  return (
...
export function LiveEarth({ size = 240 }: { size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 320 320" style={{ display: "block" }}>
      <defs>
        <clipPath id="earthBodyClipWeb">
          <circle cx={160} cy={160} r={112} />
        </clipPath>
      </defs>

      {/* Shadow */}
      <ellipse cx={160} cy={294} rx={70} ry={6} fill={SHADOW} opacity={0.35} />
      ...
      {/* Main earth body — bobs up and down */}
      <g transform={`translate(0 ${bob.toFixed(2)})`}>
        ...
        {/* Landmasses clipped to sphere */}
        <g clipPath="url(#earthBodyClipWeb)">
          <path
          ...
          <path
            ...
            strokeLinejoin="round"
          />
          <ellipse cx={160} cy={96} rx={6} ry={3.5} fill={LAND} stroke={OUTLINE} strokeWidth={3} />
        </g>
